...
function Makalat_manage() {
  ...
  const [makalInfo, setMakalInfo] = useState({ makal_title: "", makal_description: "",makal_type: "", makal_link: "", makal_img: "" });
  ...
  const handlePost = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`https://jellyfish-app-ew84k.ondigitalocean.app/api/makal`, makalInfo);
      setMakalInfo({ makal_title: "", makal_description: "",makal_type: "", makal_link: "", makal_img: "" })
  ...
  const handleRemove = async (e,id) => {
    e.preventDefault()
    try {
      const response = await axios.post(`https://jellyfish-app-ew84k.ondigitalocean.app/api/makal/${id}`);
        ...
        <form className='sign_form' onSubmit={handlePost}>
                    ...
                    <div className='sign_form_con'>
                        <div className="sign_element">
                            ...
                            <input name="makal_title" onChange={handleChange} className="sign_input"></input>
                        </div>
                        <div className="sign_element">
                            <label className="sign_label">نوع</label>
                            <input name="makal_type" onChange={handleChange} className="sign_input"></input>
                        </div>
                        <div className="sign_element">
                            <label className="sign_label">الوصف</label>
                            <input name="makal_description" onChange={handleChange} className="sign_input"></input>
                        </div>
                        <div className="sign_element">
                            <label className="sign_label">رابط المحتوي</label>
                            <input name="makal_link" onChange={handleChange} className="sign_input"></input>
                        </div>
                        <div className="sign_element">
                            <label className="sign_label">رابط الصورة</label>
                            <input name="makal_img" onChange={handleChange} className="sign_input"></input>
                        </div>
                    </div>
