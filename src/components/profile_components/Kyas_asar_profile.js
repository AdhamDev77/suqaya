...
function Kyas_asar_profile() {
  ...
  const fetchThisUser = async () => {
    try {
      ...
      const asarDetailsPromises = ids.map(id => axios.get(`https://jellyfish-app-ew84k.ondigitalocean.app/api/asar/${id}`));
  ...
  const handleDeleteAsar = async (e, asarId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const responseAsar = await axios.delete(`https://jellyfish-app-ew84k.ondigitalocean.app/api/asar/${asarId}`);
  ...
  const columns = [
    {
      ...
      selector: row => <div className="karar">
        <a className="positive_karar" href={`/asar/${row._id}`}><i class="fa-solid fa-eye"></i></a>
        {isAdmin ? (<a href={`/asar/edit/${row._id}`} className="positive_karar"><i class="fa-solid fa-pen-to-square"></i></a>) : (<></>)}
        {isAdmin ? (<button onClick={(e) => handleDeleteAsar(e, row._id)} className="negative_karar"><i class="fa-solid fa-x"></i></button>) : (<></>)}
      </div>,
    },
    {
      name: <span style={{ whiteSpace: 'normal', wordBreak: 'break-word', lineHeight: '1.5', textAlign: 'center' }}>العائد الاجتماعي على الاستثمار</span>,
      selector: row => <p className="important">{row.aed}</p> || "_"
    },
    {
      name: <span style={{ whiteSpace: 'normal', wordBreak: 'break-word', lineHeight: '1.5', textAlign: 'center' }}>صافي القيمة المجتمعية</span>,
      selector: row => <p className="important">{row.safy_kema_mogtama3ya}</p> || "_"
    },
    {
      name: 'إجمالي الموارد',
    ...
    {
      name: 'حالة المشروع',
      selector: row => <>{row.project_goals_1 ? (<p style={{ display: "flex", gap: '10px', justifyContent: "space-between", alignItems: "center" }}>جاهز <i class="fa-solid fa-circle" style={{ color: '#02e254' }}></i></p>) : (<p style={{ display: "flex", gap: '10px', justifyContent: "space-between", alignItems: "center" }}>مسودة <i class="fa-solid fa-circle" style={{ color: 'orange' }}></i></p>)}</>,
    },
  ];
