...
function Kyas_asar_settings() {
  ...
  const fetchComm = async () => {
    try {
      ...
      const responseAsar = await axios.get(`https://jellyfish-app-ew84k.ondigitalocean.app/api/asar/${asar_id}`);

      console.log(responseComm.data.approvedMembers)

      const ToggableUsersIds = responseAsar.data.toggable_users;
      const UnToggableUsersIds = responseComm.data.approvedMembers.filter(userId => !ToggableUsersIds.includes(userId));

      setCommUsers(commUsers);

      // Fetch approved members details
      const ToggableUsersPromises = ToggableUsersIds.map(async (memberId) => {
        const memberResponse = await axios.get(`https://jellyfish-app-ew84k.ondigitalocean.app/api/users/${memberId}`);
      ...
      // Fetch approved members details
      const UnToggableUsersPromises = UnToggableUsersIds.map(async (memberId) => {
        const memberResponse = await axios.get(`https://jellyfish-app-ew84k.ondigitalocean.app/api/users/${memberId}`);
        return memberResponse.data;
      });
  ...
  const handleApprove = async (e, id) => {
    ...
    try {
      const response = await axios.post(`https://jellyfish-app-ew84k.ondigitalocean.app/api/asar/toggle/${asar_id}`, { userId:id });
  ...
  const handleRemove = async (e, id) => {
    ...
    const comm_id = localStorage.getItem("_id");
    try {
      const response = await axios.post(`https://jellyfish-app-ew84k.ondigitalocean.app/api/asar/untoggle/${asar_id}`, { userId:id });
  ...
  const ApprovedColumns = [
    {
      name: 'اجراءات',
      cell: row => <div className="karar"><button onClick={(e) => handleRemove(e, row._id)} className="negative_karar"><i className="fa-solid fa-x"></i></button></div>,
    },
    {
      name: 'بريد المستخدم',
  ...
  const BasicColumns = [
    {
      name: 'اجراءات',
      cell: row => <div className="karar"><button onClick={(e) => handleApprove(e, row._id)} className="positive_karar"><i class="fa fa-plus" aria-hidden="true"></i></button></div>,
    },
    {
      name: 'بريد المستخدم',
      selector: row => row.email
