...
function Kyas_asar_show() {
  ...
  const handleBack = async (e) => {
    ...
    if (isComm) {
      navigate(`/comm/profile/${local_id}`);
    } else {
      navigate(`/profile/${local_id}`);
    }
  };
        ...
        <div className="asar_body" id="asar_10" style={{ display: "flex" }}>
          <div className="asar_header">
            ...
            <div className="asar_header_text">
              <h1 className="asar_title">عرض قياس</h1>
              <p className="asar_title_2">
                يمكنك رؤية قياس الأثر لمشروعك عن كثب من أجل تحسينه للحصول على
                أفضل النتائج في المستقبل
              </p>
            </div>
          </div>
            ...
            <div className="asar_con_con">
              <div className="asar_form_w_btns">
                <form className="asar_form table_form">
                  ...
                  <button onClick={handleBack} className="link_btn_2">
                    <i class="fa fa-arrow-left" aria-hidden="true"></i> الرجوع
                    الي الحساب
                  </button>
                </form>
              </div>
        ...
        <>
          <div className="no_kyas">
            ...
            <h1 className="no_kyas_title">
              قياس الأثر الحالي عبارة عن مسودة ولم يتم تأكيده بعد
              <br /> برجاء اكمال القياس قبل عرضه في هذة الصفحة
            </h1>
            <Link className="link_btn" to={`/asar/edit/${id}`}>
              <i class="fa-solid fa-angles-left"></i> اكمال قياس الأثر
            </Link>
          </div>
        </>
      )}
