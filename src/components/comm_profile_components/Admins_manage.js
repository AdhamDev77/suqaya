...
function User_manage({ local_id }) {
  ...
  const fetchComm = async () => {
    try {
      ...
      const filteredAdminUsers = fetchedApprovedUsers.filter(
        (user) => user.admin == true
      );
      const filteredNormalUsers = fetchedApprovedUsers.filter(
      ...
      {normalUsers ? (
        <div className="card_container_admin">
          <div className="table_con">
            <h1>المديرين</h1>
            <DataTable columns={ApprovedColumns} data={adminUsers} />
          </div>
          <div className="table_con">
            <h1>المستخدمين العاديين</h1>
            <DataTable columns={NewColumns} data={normalUsers} />
          </div>
        </div>
      ) : (
        <></>
