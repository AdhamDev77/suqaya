...
function Kyas_asar() {
  ...
  const [project_natiga, setProject_natiga] = useState([
    {
      case_id: "",
      m3ni: { m3ni_id: 1, m3ni: "", mostafed_count: "" },
      ...
      if (i === 2) {
        ...
        const allNatigaFilled = project_natiga.every((item) => {
          ...
          const isPercentageValid =
            parseInt(nesbet_ta8yyr) >= 0 && parseInt(nesbet_ta8yyr) <= 100;
        ...
        if (!allNatigaFilled) {
          project_natiga.forEach((item, index) => {
            ...
            else if (parseInt(nesbet_ta8yyr) < 0 || parseInt(nesbet_ta8yyr) > 100) {
  ...
  const handlePost = async (e) => {
    ...
    try {
      ...
      navigate(`/asar/${postResponse.data._id}`);
    } catch (error) {
      console.error("Error while posting or patching:", error);
    }
  };
  ...
  const handleRemoveM3neen = (e, index) => {
    ...
    Swal.fire({
      title: "تأكيد الحذف",
      html: "هل أنت متأكد أنك تريد حذف صاحب المصلحة؟<br>(سيتم حذف جميع النتائج المبنية على صاحب المصلحة)",
      icon: "warning",
  ...
  const handleChange2 = (index, value) => {
    ...
    localStorage.setItem("project_goals_1", JSON.stringify(newGoals));
  ...
  const handleChange3 = (index, fieldName, value) => {
    ...
    localStorage.setItem("project_goals_2", JSON.stringify(newGoals));
                ...
                <form className="asar_form" onKeyDown={(e) => e.key === 'Enter' && e.preventDefault()}>
                  ...
                  {project_natiga.map((natiga, index) => (
                    <div className="input_elements_row" key={index}>
                      ...
                      <div className="input_element_single flex_5 flex_2">
                        <input
                          type="number"
                          className="input_input"
